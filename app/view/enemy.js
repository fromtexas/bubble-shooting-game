...
function randomColor() {
  return `radial-gradient(circle, rgba(255, 255, 255, 0.3) ,rgba(${randomMe(
    255
  )}, ${randomMe(255)}, ${randomMe(255)}, 0.5))`;
}

function enemyShadow() {
  return `0 0 1rem 0 rgba(${randomMe(255)}, ${randomMe(255)}, ${randomMe(
    255
  )}, 0.3)`;
}
...
export function moveEnemies(player, Enemy, stage) {
  ...
  [...tempEnemies].forEach(item => {
    if (
      item.offsetTop > gameContainer.clientHeight ||
      item.offsetTop < 0 ||
      item.offsetLeft > gameContainer.clientWidth ||
      item.offsetLeft < 0
    ) {
